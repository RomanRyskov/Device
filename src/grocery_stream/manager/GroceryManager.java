...
public class GroceryManager {
    ...
    public List<GroceryItem> dairyFilter() {
        return groceryItems.stream()
                .filter(groceryItem -> groceryItem.getCategory() == Category.DAIRY)
    ...
    public List<GroceryItem> sortByPrice2() {
        return groceryItems.stream()
                .sorted(Comparator.comparing(GroceryItem::getPrice))
                .collect(Collectors.toList()).reversed().subList(0, 3);
    }
    public List<String> top3Names() {
        return sortByPrice2().stream()
                .map(GroceryItem::getName)
                .collect(Collectors.toList());
    }
    public void printWithDiscount() {
        groceryItems
                .forEach(groceryItem -> System.out.println(groceryItem.getName()+": "+ String.format("%.2f", groceryItem.getPrice()*0.9)+" Br"));
    }
    public boolean findLessOne(){
       return groceryItems.stream()
                .anyMatch(groceryItem -> groceryItem.getPrice()<1);
    }
    ...
    public GroceryItem findFirstVegetable(){
       Optional<GroceryItem> groceryItem = groceryItems.stream()
               .filter(groceryItem1 -> groceryItem1.getCategory() == Category.VEGETABLE)
               .findFirst();
